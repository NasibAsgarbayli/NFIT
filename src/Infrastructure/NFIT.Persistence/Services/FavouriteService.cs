...
public class FavouriteService:IFavouriteService
{
    ...
    public FavouriteService(NFITDbContext context, IHttpContextAccessor http)
    {
        _context = context;
        _http = http;
    }

    // User ID helper (Identity və ya JWT)
    ...
    public async Task<BaseResponse<string>> CreateAsync(FavouriteAddDto dto)
    {
        ...
        var existing = await _context.Favourites.FirstOrDefaultAsync(f =>
            f.UserId == userId &&
            f.GymId == dto.GymId &&
    ...
    public async Task<BaseResponse<List<FavouriteListItemDto>>> GetUserFavouriteGymsAsync()
    {
        ...
        var list = await _context.Favourites
            .Include(f => f.Gym)
            .Where(f => f.UserId == userId && !f.IsDeleted && f.GymId != null && f.Gym != null && !f.Gym.IsDeleted)
            .OrderByDescending(f => f.AddedAt)
            .AsNoTracking()
    ...
    public async Task<BaseResponse<List<FavouriteListItemDto>>> GetUserFavouriteTrainersAsync()
    {
        var userId = GetCurrentUserId();
        if (string.IsNullOrWhiteSpace(userId))
            return new BaseResponse<List<FavouriteListItemDto>>(
        ...
        if (list.Count == 0)
            return new BaseResponse<List<FavouriteListItemDto>>(
                "No favourite trainers", null, HttpStatusCode.NotFound);

        return new BaseResponse<List<FavouriteListItemDto>>(
            "Favourite trainers retrieved", list, HttpStatusCode.OK);
    }
    ...
    public async Task<BaseResponse<List<FavouriteListItemDto>>> GetUserFavouriteSupplementsAsync()
    {
        ...
        var list = await _context.Favourites
            .Include(f => f.Supplement)
            .Where(f => f.UserId == userId && !f.IsDeleted && f.SupplementId != null && f.Supplement != null && !f.Supplement.IsDeleted)
            .OrderByDescending(f => f.AddedAt)
            .AsNoTracking()
            .Select(f => new FavouriteListItemDto
            {
    ...
    public async Task<BaseResponse<string>> ClearUserFavouritesAsync()
    {
        ...
        _context.Favourites.UpdateRange(items);
        await _context.SaveChangesAsync();
